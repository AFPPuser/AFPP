...
class PPOTrainManager:
    ...
    def __init__(self,
        ...
        self.cycle = 0
        self.old_model = deepcopy(self.agent)
        self.train_n_steps = self.agent.n_pulses
    ...
    def update_learning_params(self, episode):
        ...
        self.entropy_alpha = self.initial_entropy_alpha / np.sqrt(
            1 + episode / self.decay_entropy)

        self.reward_symmetry_factor = self.initial_reward_symmetry_factor / np.sqrt(
    ...
    def single_chip_episode(self, chip, agent):
        ...
        for pulse_index in range(self.agent.n_pulses):
        ...
        returns = np.full(shape=self.agent.n_pulses, fill_value=return_val)
    ...
    def update_model(self):
        ...
        for _ in range(self.n_update_epochs):
            for _ in range(self.n_batches_per_epoch):
                ...
                episode_starts = np.random.randint(self.agent.n_pulses - self.train_n_steps + 1, size=self.batch_size)
                ...
                new_policy, critic_value = self.agent.get_policy(observations_common, observations_per_level)
                ...
                if self.agent.learn_reading_point:
                    ...
                    new_probs = new_policy.log_prob(actions)[..., :1 + self.agent.n_targets].sum(axis=-1)
                    old_log_probs = old_log_probs[..., :1 + self.agent.n_targets].sum(axis=-1)
